...
if user_prompt:
    ...
    try:
        response = requests.post(settings.BACKEND_URL+"chat/answer", json=payload)

        response.raise_for_status()
        response_json = response.json()
        print("my response", response_json)
        assistant_response = response_json.get("answer")
        tool_used = response_json.get("tool_used")
